...
import { createQuestionnaireEnvelope, loadTemplate } from "@/lib/esign/envelope-service";
import { getAdminAuth, getAdminBucket, getAdminFirestore } from "@/lib/firebase/admin";
import { col, questionnaireRequestDocId } from "@/lib/firestore/paths";
...
export async function POST(req: NextRequest) {
  ...
  const templateId =
    typeof org?.investorQuestionnaireSignableTemplateId === "string"
      ? org.investorQuestionnaireSignableTemplateId.trim()
      : "";
  if (!templateId) {
    return NextResponse.json(
      {
        error: "Your sponsor has not configured an investor questionnaire template yet.",
        mode: "stub",
      },
      { status: 400 },
    );
  ...
  if (!template) {
    return NextResponse.json({ error: "Questionnaire template not found" }, { status: 404 });
  }

  const bucket = getAdminBucket();
  const [exists] = await bucket.file(template.storagePath).exists();
  if (!exists) {
    return NextResponse.json({ error: "Questionnaire template PDF is not uploaded yet" }, { status: 503 });
  }

  const docId = questionnaireRequestDocId(ctx.orgId, dealId, ctx.user.uid);
  const existingSnap = await db.collection(col.esignEnvelopes).doc(docId).get();
  if (existingSnap.exists) {
    const e = existingSnap.data() as EsignEnvelope;
    if (e.context.kind === "deal_questionnaire") {
  ...
  try {
    const { envelope, signingUrl, awaitingSponsorPrep } = await createQuestionnaireEnvelope({
      db,
