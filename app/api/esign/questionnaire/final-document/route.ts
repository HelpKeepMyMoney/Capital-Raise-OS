...
export async function GET(req: NextRequest) {
  ...
  const docId = questionnaireRequestDocId(ctx.orgId, dealId, targetUserId);
  const db = getAdminFirestore();
  const envSnap = await db.collection(col.esignEnvelopes).doc(docId).get();
  if (!envSnap.exists) {
    return NextResponse.json({ error: "Signed questionnaire not found" }, { status: 404 });
  }
  const env = { id: envSnap.id, ...(envSnap.data() as Omit<EsignEnvelope, "id">) };
  if (env.organizationId !== ctx.orgId || env.context.kind !== "deal_questionnaire") {
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(buf), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="questionnaire-${dealId}-signed.pdf"`,
        "Cache-Control": "private, no-store",
      },
    });
  } catch (e) {
    console.error("[esign questionnaire final-document]", e);
