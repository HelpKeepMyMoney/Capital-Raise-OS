...
- **Investor portal home (`app/(shell)/portal/page.tsx`):** Lists per-deal subscription signing state via **`getSigningRequest`** — **Signed** rows link to **`GET /api/esign/subscription/final-document`**, in-flight rows show status hints; copy points to **My commitments** and deal pages.
- **Deal detail (guest/investor, `app/(shell)/deals/[id]/page.tsx`, `components/deals/deal-detail-shell.tsx`, `deal-title-hero.tsx`, `request-subscription-packet-button.tsx`):** When the subscription envelope is **completed**, hero and sticky bar show **Download subscription packet** (`target="_blank"`) instead of the request action.
...
- `app/api/` — session auth, **`platform-admin`**, discovery, outreach, data room (**`rooms/[roomId]/nda-eligible-investors`** for mutual-NDA recipient picker), deals, tasks, organizations, invitations, AI chat, PayPal billing, webhooks, **`esign/*`** (templates, template file, envelopes, sign-session, sign-complete, subscription create), **`organizations/[id]/esign-settings`**
