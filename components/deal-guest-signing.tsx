...
export function DealGuestSigning(props: {
  ...
  async function requestPacket() {
    ...
    try {
      ...
      setRow({
        ...
        sponsorTurnAfterLpSigned: Boolean(json.sponsorTurnAfterLpSigned),
  ...
  const showRequest =
    ...
    row.status === "declined" ||
    row.status === "error" ||
    (row.awaitingSponsorPrep && !row.sponsorTurnAfterLpSigned) ||
    (row.status === "sent" && !row.signingUrl && !row.awaitingSponsorPrep);
  ...
  return (
    <div className="space-y-4">
      <Card id="subscription-documents" className="rounded-2xl border-border/80 shadow-md">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            The subscription agreement records your intent to invest and the terms you are accepting for this offering.
            Completing it helps move your allocation toward closing alongside the rest of your diligence.
          </p>
        </CardHeader>
        <CardContent className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
          {row?.awaitingSponsorPrep && row.sponsorTurnAfterLpSigned ? (
            <div className="w-full rounded-xl border border-border/80 bg-muted/30 p-3 text-sm">
              <p className="font-medium text-foreground">Awaiting sponsor signature</p>
              <p className="mt-1 text-muted-foreground">
                You&apos;ve completed your part of the subscription agreement. Your sponsor team has been emailed a
                link to add their signature. The final PDF will be available here once everyone has signed.
              </p>
              {row.sponsorSigningUrl ? (
                <details className="mt-2" open={sponsorNotifyOk === false}>
                  <summary className="cursor-pointer text-xs text-muted-foreground underline-offset-4 hover:underline">
                    Share sponsor signing link manually
                  </summary>
      ...
      {(props.questionnaireConfigured || qRow) ? (
        <Card className="rounded-2xl border-border/80 shadow-md">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              The investor questionnaire collects information used to confirm eligibility, suitability, and fit for this
              offering. Your answers become part of a signed packet—take a moment to respond accurately.
            </p>
          </CardHeader>
          <CardContent className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            {qRow?.awaitingSponsorPrep ? (
